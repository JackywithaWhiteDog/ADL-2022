...
if __name__ == "__main__":
    ...
    dialogues = [
        f"{i.replace(' @ ', ' ')} {p}"
        for i, p in zip(test_dataset['inputs'], predictions)
    ]
    output_dialogue_file = os.path.join(training_args.output_dir, "generated_dialogues.txt")
    with open(output_dialogue_file, "w", encoding="utf-8") as writer:
        writer.write("\n".join(dialogues))
