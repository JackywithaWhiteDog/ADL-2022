    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step +1) % args.validate_steps == 0:
                ...
                result = evaluate(args, model, tokenizer, args.evaluate_metrics, prefix=epoch, save=False)
                #logger.info(result)
...
def evaluate(args, model, tokenizer, evaluate_metrics="ppl", prefix='0', save=True):
...
def save_generation(args, results, prefix='0'):
    results = [post_processing(line) for line in results]
...
def post_processing(line):
    ignore_tokens = ['<|bos|>', '<|pad|>', '<endoftext>']
...
def main():
    ...
    parser.add_argument('--sampling_topp', type=float, default=-1.0,
                        help="Top-p")
    parser.add_argument('--temerature', type=float, default=1.,
                        help="Temperature")
